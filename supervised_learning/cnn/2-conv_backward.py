...
def conv_backward(dZ, A_prev, W, b, padding="same", stride=(1, 1)):
    ...
    Parameters:
    dZ (numpy.ndarray): Partial derivatives w.r.t the unactivated output of shape (m, h_new, w_new, c_new).
    A_prev (numpy.ndarray): Output of the previous layer of shape (m, h_prev, w_prev, c_prev).
    ...
    for i in range(h_new):
        for j in range(w_new):
            for k in range(c_new):
                ...
                dA_prev_padded[:, i * sh:i * sh + kh, j * sw:j * sw + kw, :] += dZ[:, i, j, k][:, None, None, None] * W[:, :, :, k]
                dW[:, :, :, k] += np.sum(slice_A * dZ[:, i, j, k][:, None, None, None], axis=0)
    
    dA_prev = dA_prev_padded[:, ph:-ph, pw:-pw, :] if padding == "same" else dA_prev_padded
