...
# Neuron class definition directly in 6-main.py
class Neuron:
    """
...
# Example usage (without any matplotlib for plotting)
if __name__ == "__main__":
    # Example input data X (nx=2, m=4) and corresponding labels Y (1, m)
    X = np.array([[1, 2, 3, 4], [2, 3, 4, 5]])  # 2 features, 4 examples
    Y = np.array([[0, 1, 0, 1]])  # Labels

    # Initialize and train the neuron
    neuron = Neuron(2)  # Initialize neuron with 2 input features
    predictions, cost = neuron.train(X, Y, iterations=10000, alpha=0.1)
